...
function submit_check_first(){
		...
		$.ajax({
			    type: 'POST',
			    url: 'http://user.mydroid.io/viloos/viloosfrontend/api_registration.php',
			    data: {
...
function submit_to_joomla2(name,username,password1,password2,email1,email2,option,task){
		$.ajax({
			    type: 'POST',
			    url: 'http://user.mydroid.io/viloos/index.php', //url of joomla website
			    data: {
			    	'option': option,
			    	'task': task,
			    	'jform[name]':name,
...
// START CONNECT TO JOOMLA BACKEND
	...
	function submit_to_joomla(){ 
		...
		$.ajax({
                type: "POST",
                url: "http://user.mydroid.io/viloos/viloosfrontend/api_login.php",
                data: {
                    username: jform_username,
...
// END CONNECT TO JOOMLA BACKEND
    //  START PULL DATA FROM MYSQL for FEEDBACK
     $( document ).ready(function() {
			    $.ajax({
				    url: 'http://user.mydroid.io/viloos/api.php?private_key=e4ed5acb3a53e3935a1a8f1676fb111de2a4842a',
				    type: "get",
				    dataType: "json",
				    success: function(response) {
    ...
    $(document).on("click", "#sendfeedback", function(evt)
    {
      ...
      var $form = $( this ),
      url = "http://user.mydroid.io/viloos/viloos.php";
